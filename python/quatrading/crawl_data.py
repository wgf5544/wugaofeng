...
def crawl(code='000001.SS', source='yahoo', **kw) -> DataFrame:
    from_year = 0
    from_month = 1
    from_day = 1
    to_year = 0
    ...
    if kw:
        if 'year' not in kw:
            raise KeyError("has not 'year' key ...")
        from_year = kw.get('year', 0)
        from_month = kw.get('month', 1)
        from_day = kw.get('day', 1)
        to_year = kw.get('toyear', 0)
        to_month = kw.get('tomonth', 1)
        to_day = kw.get('today', 1)
    else:
        raise KeyError("no 'year' 'month'...")

    if to_year == 0:
        df_stockload = web.DataReader(code, source, datetime.datetime(from_year, from_month, from_day),
                                      datetime.date.today())
    else:
        df_stockload = web.DataReader(code, source, datetime.datetime(from_year, from_month, from_day),
                                      datetime.datetime(to_year, to_month, to_day))
    return df_stockload